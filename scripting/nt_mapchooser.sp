...
public void OnPluginStart()
{
	...
	g_Cvar_NoVoteMode = CreateConVar("sm_mapvote_novote", "0", "Specifies whether or not MapChooser should pick a map if no votes are received.", _, true, 0.0, true, 1.0);
	...
	g_Cvar_RunOffPercent = CreateConVar("sm_mapvote_runoffpercent", "33", "If winning choice has less than this percent of votes, hold a runoff", _, true, 0.0, true, 100.0);
...
public Action Event_RoundStart(Event event, const char[] name, bool dontBroadcast)
{
	...
	int JinScore = GetTeamScore(TEAM_Jin);
	int NSFScore = GetTeamScore(TEAM_NSF);
	
	if(JinScore >= winlimit || NSFScore >= winlimit)
	{
		ChangeMapNow = true;
	}
	...
	if (g_Cvar_Winlimit != INVALID_HANDLE)
	{
		...
		{
			if (JinScore >= (winlimit - GetConVarInt(g_Cvar_StartRounds)))
			{
		...
		{
			if (NSFScore >= (winlimit - GetConVarInt(g_Cvar_StartRounds)))
			{
				InitiateVote(MapChange_MapEnd, null);
			}
		}
